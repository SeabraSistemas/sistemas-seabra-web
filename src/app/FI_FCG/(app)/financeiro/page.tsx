...
export default async function FinanceiroPage() {
  await exigirSessao();
  const { vendas, baixas, abortos, lancamentos } = await getDadosFinanceiro();
  const { eventos, orfaos } = montarEventos(vendas.itens, baixas.itens, abortos.itens, lancamentos.itens);

  const carregadoEm = [vendas, baixas, abortos, lancamentos]
    .map((l) => l.carregadoEm)
  ...
  const dados: PacoteFinanceiro = {
    ...
    stale: vendas.stale || baixas.stale || abortos.stale || lancamentos.stale,
    carregadoEm,
  };
