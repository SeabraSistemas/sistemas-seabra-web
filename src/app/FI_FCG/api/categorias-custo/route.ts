...
import { criarCategoriaCusto, excluirCategoriaCusto, renomearCategoriaCusto } from '@/lib/fi-fcg/mutations';
...
export async function POST(request: Request) {
  ...
  const { ok, id } = await criarCategoriaCusto({ nome });
...
export async function PATCH(request: Request) {
  ...
  const ok = await renomearCategoriaCusto(id.trim(), { nome });
...
export async function DELETE(request: Request) {
  ...
  const ok = await excluirCategoriaCusto(id.trim());
  if (!ok) return NextResponse.json({ erro: 'categoria não encontrada ou falha ao apagar' }, { status: 404 });
