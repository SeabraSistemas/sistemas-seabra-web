...
import { TRIAL_ALERTA_DIAS, carregarBalde } from '@/lib/adm/areas/risco';
...
export default async function RiscoPage() {
  ...
  const [silencioRes, inadimplentesRes, trialRes, semPagamentoRes, cobrancasRes] = await Promise.all([
    carregarBalde('silencio', agora),
    carregarBalde('inadimplente', agora),
    carregarBalde('trial', agora),
    carregarBalde('sem_pagamento', agora),
    listarCobrancas(),
  ]);
  ...
  if (!inadimplentesRes.ok) return <EstadoVazio resultado={inadimplentesRes} />;
  if (!trialRes.ok) return <EstadoVazio resultado={trialRes} />;
  if (!semPagamentoRes.ok) return <EstadoVazio resultado={semPagamentoRes} />;
  ...
  const inadimplentes = inadimplentesRes.dados;
  ...
  const semPagamento = semPagamentoRes.dados;
    ...
    <div className="flex flex-col gap-6">
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        <div className="flex flex-col gap-2">
          ...
          />
          <ExportBotoes tela="risco" parametros={{ balde: 'silencio' }} contagem={silencio.length} className="px-1" />
        </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <ExportBotoes
            tela="risco"
            parametros={{ balde: 'inadimplente' }}
            contagem={inadimplentes.length}
            className="px-1"
          />
        </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          />
          <ExportBotoes tela="risco" parametros={{ balde: 'trial' }} contagem={trial.length} className="px-1" />
        </div>
      </section>
      ...
      <section className="flex flex-col gap-3 rounded-2xl border border-dashed border-border bg-card/40 p-4">
        ...
        <ExportBotoes
          tela="risco"
          parametros={{ balde: 'sem_pagamento' }}
          contagem={semPagamento.length}
        />
      </section>
