...
export default async function SanidadePage({
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {/*
        Óbitos ganham tela própria: aqui eles são contagem e taxa de 12 meses, e
        ...
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          <div>
            ...
            <p className="mt-0.5 text-xs text-muted-foreground">
              Quem morreu, com que idade e de quê: faixa etária, ranking de suspeitas e a lista
              óbito a óbito.
            </p>
          </div>
          <Link
            href={`/adm/u/${usuarioId}/sanidade/obitos${selecao == null ? '' : `?prop=${selecao}`}`}
      ...
      <section className="rounded-2xl border border-border bg-card p-4">
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          <div>
            ...
            <p className="mt-0.5 text-xs text-muted-foreground">
              FAMACHA, escore corporal, casco, protocolo sanitário e os outros — cada um com o seu
              filtro. Uma ida ao curral mede várias coisas de uma vez, e a contagem separa manejo de
              lançamento.
            </p>
          </div>
          <Link
            href={`/adm/u/${usuarioId}/sanidade/manejos${selecao == null ? '' : `?prop=${selecao}`}`}
            className="shrink-0 rounded-full border border-primary bg-primary px-3 py-1 text-sm text-primary-foreground transition-opacity hover:opacity-90"
          >
            Abrir manejos
          </Link>
      ...
      <section className="rounded-2xl border border-border bg-card p-4">
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          <div>
            ...
            <p className="mt-0.5 text-xs text-muted-foreground">
              Cruza doença com morte: de cada dez animais que tiveram um caso de X, quantos morreram
              logo depois. É a conversa de prioridade de vacina com o número do próprio cliente.
            </p>
          </div>
          <Link
            href={`/adm/u/${usuarioId}/sanidade/clinica${selecao == null ? '' : `?prop=${selecao}`}`}
            className="shrink-0 rounded-full border border-primary bg-primary px-3 py-1 text-sm text-primary-foreground transition-opacity hover:opacity-90"
          >
            Abrir clínica
          </Link>
